...
/*   Updated: 2025/03/13 11:07:03 by znajdaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_philo_eat(t_philo *p)
{
	...
	ft_msleep(p->data->teat, &(p->is_done), &(p->lis_done));
...
{
	ft_dely(p->data->start_time, &(p->is_done), &(p->lis_done));
	if (p->data->philo_num == 1)
	{
		sem_wait(p->data->forks.addr);
		ft_print_msg_status(p);
		ft_msleep(p->data->tdie + 10, &(p->is_done), &(p->lis_done));
		return (ERR_SUCCESS);
	}
	if (p->id % 2 == 0)
	...
	while (true)
	{
		...
		ft_msleep(p->data->tsleep, &(p->is_done), &(p->lis_done));
